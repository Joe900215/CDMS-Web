...
{
    internal class DeleteProjectMenu : ExWebMenu
    {
        ...
        public override enWebMenuState MeasureMenuState()
        {
            ...
            try
            {
                ...
                if (project.O_iuser1 != null && !base.LoginUser.IsAdmin)
                {
                    MenuState = enWebMenuState.Disabled; 
                }
                else if ((project.O_type == enProjectType.GlobCustom || project.O_type == enProjectType.UserCustom || 
                    project.O_type == enProjectType.GlobSearch ||project.ShortProject != null) &&
                    base.LoginUser.IsAdmin)
                {

                    MenuState = enWebMenuState.Enabled;
                }
                else if (project.O_type == enProjectType.Local || project.O_type == enProjectType.UserCustom ||
                    project.O_type == enProjectType.GlobSearch || project.ShortProject != null)
                {
                    bool hasPDeleteRight = ProjectController.GetProjectPDeleteRight(project, base.Sid);
