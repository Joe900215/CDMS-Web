    ...
    public class SelectUnit
    {
        public static JObject GetSelectUnitList(string sid, string ProjectKeyword, string Filter,string UnitType)
        {
            ...
            try
            {
                ...
                //判断要获取哪些单位
                bool bGetUnit = true, bGetCommCode = true, bGetDepartment = true;
                if (UnitType == "CommCode")
                {
                    bGetUnit = false;
                    bGetDepartment = false;
                }
                ...
                if (bGetCommCode)
                {
                    ...
                    AttrData data;
                    string commDesc = "";
                    if ((data = prjProject.GetAttrDataByKeyWord("PRO_COMNAME")) != null) {
                        ...
                        if ((data = prjProject.GetAttrDataByKeyWord("RPO_ONSHORE")) != null)
                    {
                        ...
                        if (!string.IsNullOrEmpty(strData))
                        {
                            JObject joData = new JObject(
                                ...
                                new JProperty("unitDesc", string.IsNullOrEmpty(commDesc)?"OnShore":commDesc)
                                );
                            jaData.Add(joData);
                        }
                    }
                    if ((data = prjProject.GetAttrDataByKeyWord("RPO_OFFSHORE")) != null)
                    {
                        ...
                        if (!string.IsNullOrEmpty(strData))
                        {
                            JObject joData = new JObject(
                                ...
                                new JProperty("unitId", data.DefnID),
                                new JProperty("unitCode", data.ToString),
                                new JProperty("unitDesc", string.IsNullOrEmpty(commDesc) ? "OffShore" : commDesc)
                                );
                            jaData.Add(joData);
                        }
                    }
