    ...
    public class EditCompanyInfo
    {
        ...
        public static JObject CreateCompany(string sid, string ProjectKeyword, string companyAttrJson)
        {
            ...
            try
            {
                ...
                #region 获取传递过来的属性参数
                //获取传递过来的属性参数
                JArray jaAttr = (JArray)JsonConvert.DeserializeObject(companyAttrJson);
                    ...
                    if (cdProject != null)
                    {
                        ...
                        if (txProject != null)
                        {
                            ...
                            if (project != null)
                            {
                                ...
                                //把收文单位文控用户，添加到项目的通信用户项目组
                                User secUser = CommonFunction.GetUserByFullName(dbsource,strSecretarilman);
                                if (secUser != null)
                                {
                                    Server.Group mainProjGroup = dbsource.AllGroupList.Find(g => g.Code == prjProject.Code + "_ComGroup");
                                    if (mainProjGroup != null)
                                    {
                                        mainProjGroup.AddUser(secUser);
        ...
        public static JObject DeleteCompany(string sid, string ProjectKeyword, string CompanyCode, string CompanyId)
        {
            ...
            try
            {
                ...
                foreach (DictData data6 in dictDataList)
                {
                    ...
                    if (!string.IsNullOrEmpty(data6.O_sValue1) && data6.O_sValue1 == strProjCode
                        && data6.O_Code == CompanyCode && data6.O_ID == companyId)
                    {
                        data6.Delete();
                        break;
                    }
