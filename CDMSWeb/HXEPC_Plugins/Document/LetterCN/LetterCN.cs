    ...
    public class LetterCN
    {
        ...
        public static JObject DraftLetterCN(string sid, string ProjectKeyword, string DocAttrJson, string CataAttrJson, string FileListJson, string PreDocKeyword)
        {
            ...
            try
            {
                ...
                foreach (JObject joAttr in jaAttr)
                {
                    ...
                    //获取预制发文编码
                    else if (strName == "preSendCode") preSendCode = strValue;
                ...
                Doc docItem = null;
                if (string.IsNullOrEmpty(PreDocKeyword))
                {
                    //新建一个文档
                    docItem = m_Project.NewDoc(filename + ".docx", filename, "", docTempDefn);
                    if (docItem == null)
                    {
                        reJo.msg = "新建信函出错！";
                        return reJo.Value;
                    ...
                    Doc ddoc = dbsource.GetDocByKeyWord(PreDocKeyword);
                    if (ddoc == null)
                    {
                        reJo.msg = "获取预占号文档出错！";
                        return reJo.Value;
                    }
                    docItem = ddoc.ShortCutDoc == null ? ddoc : ddoc.ShortCutDoc;
                    sendCode = docItem.O_itemname;
                    filename = sendCode + " " + title;
                    ...
                    docItem.O_itemname = filename;
                    docItem.O_filename = filename + ".docx";

                    docItem.Modify();
                    //docItem.GetAttrDataByKeyWord("");
                }
                ...
                //文文档模板
                caDoc.CA_ATTRTEMP = "LETTERFILE";
                //文件编码
                caDoc.CA_FILECODE = fileCode;
                //主送
                caDoc.CA_MAINFEEDER = mainFeeder;
                //发送方
                caDoc.CA_SENDER = sender;
                //发文编码
                if (string.IsNullOrEmpty(caDoc.CA_SENDCODE))
                {
                ...
                //紧急程度
                caDoc.CA_URGENTDEGREE = urgency;
                //发送日期
                caDoc.CA_SENDDATE = sendDate;
                //保密等级
                caDoc.CA_SECRETGRADE = seculevel;
                //保密期限
                caDoc.CA_SECRETTERM = secrTerm;
                
                //获取是否需要回复
                caDoc.CA_IFREPLY = needreply;
                //回复日期
                caDoc.CA_REPLYDATE = replyDate;
                //标题
                caDoc.CA_TITLE = title;
                //内容
                caDoc.CA_CONTENT = content;
                //文件列表
                caDoc.CA_ENCLOSURE = FileListJson;
                //校审级数（审批路径）
                ...
                //需要回复时，填写回复日期
                if (needreply == "是")
                    strReplydate = replydate.ToShortDateString().ToString().Replace("-", ".").Replace("/", ".");
            ...
            string ApprovPath,string UserList, string SendUnitCode)
        {
            ...
            try
            {
                ...
                //记录文档所在目录，是运营管理类还是项目管理类
                Server.Group secGroup = null;
                ...
                if (rootProj == null)
                {
                    ...
                    {
                        //从组织机构里面查找文控
                        gp = dbsource.GetGroupByName(SendUnitCode);
                        if (gp == null)
                        {
                            reJo.msg = "部门文控未设置！";
                            return reJo.Value;
                        ...
                        secGroup = CommonFunction.GetSecGroupByUnitCode(dbsource, SendUnitCode);
                ...
                else
                {
                    ...
                    secGroup = CommonFunction.GetGroupByFullName(dbsource, secUserList);
                ...
                if (secGroup==null || secGroup.UserList.Count <= 0)
                {
                    reJo.msg = "获取文控错误，自动启动流程失败！请手动启动流程";
                    return reJo.Value;
                }
               ...
               string  approvUserList = "";
                Server.Group approvGroup = new Server.Group();

                if (isProjectDoc == false)
                {
                    //运营管理类
                    #region 从组织机构里面获取部长
                    approvGroup = CommonFunction.GetGroupByDesc(dbsource, SendUnitCode, "部长");
                    //gp = dbsource.GetGroupByName(SendUnitCode);
                    //if (gp != null)
                    //{
                    ...
                    #region 从项目目录查找领导
                    AttrData ldData;
                    if ((ldData = rootProj.GetAttrDataByKeyWord("DESIGNDIRECTOR")) != null)
                    {
                        approvUserList = ldData.ToString;
...

                {
                    ...
                    foreach (string strObj in strArray)
                    {
                        ...
                        {
                            Doc mddoc = (Doc)obj;
                            Doc mdoc = mddoc.ShortCutDoc == null ? mddoc : mddoc.ShortCutDoc;
                            m_DocList.Add(mdoc);
                        }
                    }
                    #endregion
                    ...
                    if (ApprovPath != "二级-编批")
                    {
                        state = flow.WorkStateList.Find(wsx => (wsx.Code == strCheckStateCode) && (wsx.CheckGroup.AllUserList.Count == 0));
                        if (state == null)
                        {
                    ...
                    //当审批路径是五级审批的时候，放置审核状态人员
                    if (isProjectDoc == false)
                    {
                        if (ApprovPath == "五级-编校审定批")
                        {
                            state = flow.WorkStateList.Find(wsx => (wsx.Code == "AUDIT") && (wsx.CheckGroup.AllUserList.Count == 0));
                            if (state == null)
                            {

                                AVEVA.CDMS.Server.Group auGroup = CommonFunction.GetGroupByDesc(dbsource, SendUnitCode, "部长助理");
                                if (auGroup != null)
                                {
                                    DefWorkState defWorkState = flow.DefWorkFlow.DefWorkStateList.Find(dwsx => dwsx.O_Code == "AUDIT");
                                    state = flow.NewWorkState(defWorkState);
                                    state.SaveSelectUser(auGroup);
                    ...
                    #region 放置批准状态人员
                    state = flow.WorkStateList.Find(wsx => (wsx.Code == "APPROV") && (wsx.CheckGroup.AllUserList.Count == 0));
                    if (state == null)
                    {
                        DefWorkState defWorkState = flow.DefWorkFlow.DefWorkStateList.Find(dwsx => dwsx.O_Code == "APPROV");
                        state = flow.NewWorkState(defWorkState);
                        state.SaveSelectUser(approvGroup);
                    ...
                    if (isProjectDoc == false)
                    {
                        ...
                        if ((data = doc.GetAttrDataByKeyWord("CA_MAINFEEDERCODE")) != null)
                        {
                            ...
                            //根据部门编码获取文控
                            recGroup = CommonFunction.GetSecGroupByUnitCode(dbsource, recerCode);

                            if (recGroup == null || recGroup.AllGroupList == null || recGroup.AllUserList.Count == 0)
                            {
                                //从项目里面获取项目的文控
                                #region 从项目里面获取项目的文控
                                bool isToProj = false;
                                ...
                                if (isToProj) {
                                    AttrData pdata = doc.GetAttrDataByKeyWord("CA_SENDCODE");
                                    string sendcode = pdata.ToString;

                                    string projCode = sendcode.Substring(0, sendcode.IndexOf("-"));

                                    //从根目录往下查找，获取项目文控
                                    recGroup = CommonFunction.GetProjectSecUser(doc.dBSource, projCode);
                                }
                                ...
                                //        ////获取项目目录
                                //        //Project rProj = doc.dBSource.RootLocalProjectList.Find(itemProj => itemProj.TempDefn.KeyWord == "PRODOCUMENTADMIN");
                                ...
                                //        //    doc.dBSource.GetUserByCode(strUser);
                                //        //    object obj = dbsource.GetUserByName(strUser);
                    ...
                    {
                        //项目管理类
                        //1.判断是发给项目部，还是发给参建单位，还是发给运营管理里面的项目部门
                        //2.（忽略，参建单位发给项目部，用收文流程）如果是发给项目部，就获取项目文控
                        //3.如果是发给参建单位，就获取参建单位的文控
                        //4.如果是运营管理里面的发给项目部门，就获取项目部门的文控
                        ...
                        //获取项目文控
                        recGroup = CommonFunction.GetSecGroupByUnitCode(dbsource,recUnitCode);

                        //如果项目部门里面找不到文控，再到参建单位里面找文控
                        if (recGroup == null)
                        {
                            User recUser = dbsource.GetUserByName(recUnitCode);
                            if (recUser != null || recGroup.AllGroupList == null || recGroup.AllUserList.Count == 0)
                            {
                                recGroup = new Server.Group();
                                recGroup.AddUser(recUser);
                            }
                        }
        ...
        public static JObject GetLetterCNNumber(string sid, string ProjectCode, string SendCompany, string RecCompany)
        {
            ...
            try
            {
                ...
                string runNum = Document.getDocTempNumber(dbsource, "S", "LET"); //, SendCompany, RecCompany);
