...
namespace AVEVA.CDMS.HXEPC_Plugins
{
    ...
    public class TransmittalCN
    {
        ...
        public static JObject GetDraftTransmittalCNDefault(string sid, string ProjectKeyword, string DraftOnProject)
        {
            ...
            //    DraftGetDefault.GetProjectInfo(m_Project, ref ProjectDesc, 
            //        ref RootProjectCode, ref RootProjectDesc, ref bDraftOnProject);
        ...
        public static JObject DraftTransmittalCN(string sid, string ProjectKeyword, string DocAttrJson, string CataAttrJson, string FileListJson)
        {
            ...
            try
            {
                ...
                else if (string.IsNullOrEmpty(transmode))
                {
                    reJo.msg = "请填写传递方式！";
                    return reJo.Value;
                }
                ...
                if (!string.IsNullOrEmpty(FileListJson))
                {
                    ...
                    foreach (JObject joAttr in jaFiles)
                    {
                        ...
                        string strReceiptCode = joAttr["rc"].ToString();//收文编码
                        string strOriginalShare = joAttr["os"].ToString();//状态
                        string strCopyShare = joAttr["cs"].ToString();//状态
                        string strScanShare = joAttr["ss"].ToString();//状态
                        string strElecShare = joAttr["es"].ToString();//状态
                        ...
                        LetterAttaFile afItem = new LetterAttaFile()
                        {
                            ...
                            Page = strFilePage,
                            Edition = strEdition,
                            ReceiptCode = strReceiptCode,
                            OriginalShare = strOriginalShare,
                            CopyShare = strCopyShare,
                            ScanShare = strScanShare,
                            ElecShare = strElecShare,
                            Remark = strRemark
                        };
                ...
                //文文档模板
                caDoc.CA_ATTRTEMP= "FILETRANSMIT";
                ...
                //发文编码
                caDoc.CA_SENDCODE = documentCode;
                ...
                //传递方式
                caDoc.CA_TRANSMITMETHOD = transmode;
                //传递方式的补充说明
                caDoc.CA_TRANSMITMETHODSUPP = transmodeSupp;
                //提交目的
                caDoc.CA_SUBMISSIONOBJ = purpose;
                //提交目的补充说明
                caDoc.CA_SUBMISSIONOBJSUPP = purposeSupp;
                //标题
                caDoc.CA_TITLE = title;
                //摘要
                caDoc.CA_ABSTRACT = content;
                //校审级数（审批路径）
                ...
                foreach (LetterAttaFile file in attaFiles)
                {
                    ...
                    list3.Add(file.ReceiptCode);//收文编码
                    list3.Add(file.OriginalShare);//原件份数
                    list3.Add(file.CopyShare);//复印件份数
                    list3.Add(file.ScanShare);//扫描件份数
                    list3.Add(file.ElecShare);//电子文件份数
                    list3.Add(file.Remark);//备注
    ...
    internal struct LetterAttaFile
    {
        ...
        public string OriginalShare { get; set; }
        public string CopyShare { get; set; }
        public string ScanShare { get; set; }

        public string ElecShare { get; set; }
    }
}
