namespace AVEVA.CDMS.WTMS_Plugins
{
    ...
    public class CommonFunction
    {
                ...
                if (wf.DefWorkFlow.O_Code == "COMMUNICATIONWORKFLOW")
                {
                    ...
                    if (wsDCList.Count > 0 && wsList.Count > 0 && wsDCList.Count == wsList.Count)
                    {
                        ...
                        if (mtoUser.Count > 0)
                        {
                            ...
                            string str11 = wf.doc.Project.GetValueByKeyWord("WTMS_DOCUMENTSYSTEM_DESC");
            ...
            try
            {
                ...
                else
                {
                    ...
                    foreach (Project proj in m_RootProject.AllProjectList)
                    {
                        try
                        {
                            if (proj.TempDefn.KeyWord == "WTMS_DOCUMENTSYSTEM")
        ...
        /// <returns></returns>
        internal static List<User> GetUserListByDesc(DBSource dbsource,string UserListDesc) {
            List<User> resultUserList = new List<User>();

            string[] UserArray = (string.IsNullOrEmpty(UserListDesc) ? "" : UserListDesc).Split(new char[] { ',' }, StringSplitOptions.RemoveEmptyEntries);
