Ext.define('Ext.ux.YDForm.Group._EditProjectGroup', {
    //编辑项目用户组
    extend: 'Ext.container.Container',
    alias: 'widget.editProjectGroup',
    layout: 'fit',
    resultvalue: '', mainPanelId: '',
    projectKeyword: '', groupKeyword: '', groupDesc: '',
    initComponent: function () {
        var me = this;


        ////定义用户组选择Panel
        me._selectGroupUserPanel = Ext.create('Ext.ux.YDForm.User._SelectUserGroupPanel', { GroupType: 'Project', Filter: me.groupDesc });
        ...
        me._selectGroupUserPanel.grouppicker.on("render", function () {
            setInterval(function () {
                var gpicker = me._selectGroupUserPanel.grouppicker;
                if (gpicker.getRootNode().firstChild != undefined) {
                    var record = gpicker.getRootNode().firstChild;

                    var fnode =gpicker.store.getNodeById(record.data.id);
                    gpicker.getSelectionModel().select(fnode, true);
                }
            }, 500);
    ...
    sendDropUserList: function (groupKeyword, userNodes, userIndex) {
        ...
        Ext.Ajax.request({
            ...
            params: {
                ...
                sid: localStorage.getItem("sid"), groupKeyword: gKeyword,
    ...
    //添加用户
    sendAddUserList: function (groupKeyword, userKeyword, userName, userIndex) {
        var me = this;
        //var userKeyword = userNodes[userIndex].data.id;
        //var userName = userNodes[userIndex].data.text;
        var gKeyword = groupKeyword;
        ...
        Ext.Ajax.request({
            ...
            params: {
                ...
                sid: localStorage.getItem("sid"), groupKeyword: groupKeyword,
                userKeyword: userKeyword
            },
            success: function (response, options) {
                var res = Ext.JSON.decode(response.responseText, true);
