...
Ext.define('Ext.ux.gcz.Doc._MainDocGrid', {
    ...
    initComponent: function () {
        ...
        me._docListTbar = Ext.create('Ext.toolbar.Toolbar', {
            ...
            items: [
                ...
                {
                    iconCls: "file-preview", text: '打开图纸', scope: me, tooltip: '打开图纸', listeners: {
                        "click": function (btn, e, eOpts) {
                ...
                {
                    iconCls: "file-preview", text: '参照', scope: me, tooltip: '参照', listeners: {
                        "click": function (btn, e, eOpts) {
    ...
    OpenCAD: function () {
        ...
        //判断浏览器的类型，是否使用cefSharp浏览
        if (_browserName == "cefSharp") {
            //Ext.Msg.alert("错误信息", "调用参照！");
            //var gridView = Ext.getCmp(mainPanelId).up('_mainProjectView').down('_mainDocGrid');
            ...
            if (rs !== null && rs.length > 0) {
                ...
                if (_browserName == "cefSharp") {
                    //调用C#定义事件
                    jsObj.OnDocGridReferCAD(localStorage.getItem("sid"), Keyword);
                    return;
                    //continue;
                }
                //}
            ...
            if (rs !== null && rs.length > 0) {
                ...
                //if (_browserName == "cefSharp") {
                //调用C#定义事件
                //Ext.Msg.alert("错误信息", "请在CAD里调用参照！");
                //window.external.MyMessageBox(_browserName)

                var index = me.maindocgrid.store.indexOfId(rs[i].data.Keyword);//extjs现时每行的ID得到现时行号
                //var index = rs[0].get('id');//获得选中第一项的id列的值rs.
                //找到文件状态所在图标的DOM对象
                var cn = me.maindocgrid.el.dom.childNodes["0"].childNodes[2].childNodes["0"].childNodes[1].childNodes[index ];
                ...
                //["0"].childNodes[2].childNodes["0"].childNodes[1].childNodes[6]
                //获取原来的Class状态
                var oldClass = viewImg.getAttribute("class");

                window.external.OnGridOpenCAD(localStorage.getItem("sid"), Keyword, me.maindocgrid.id, index, oldClass)
                return;
                //continue;
                //}
                //}
    ...
    ReferCAD: function () {
        ...
        if (_browserName == "cefSharp") {
            ...
            if (rs !== null && rs.length > 0) {
                ...
                //Ext.Msg.alert("错误信息", "请在CAD里调用参照！");
                //window.external.MyMessageBox(_browserName)
                window.external.OnDocGridReferCAD(localStorage.getItem("sid"), Keyword)
                return;
                //continue;
                //}
                //}
