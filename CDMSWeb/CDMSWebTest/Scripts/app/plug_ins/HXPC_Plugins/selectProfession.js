//选择项目设总
Ext.define('Ext.plug_ins.HXPC_Plugins.selectProfession', {
    extend: 'Ext.container.Container',
    alias: 'widget.selectProfession',
    //layout: "border",
    layout: 'fit',
    resultvalue: '', mainPanelId: '', projectDesc: '',
    projectKeyword: '',startAtWfBtn:'false',
    ...
    initComponent: function () {
        ...
        me.designPhaseCombo = Ext.create("Ext.form.field.ComboBox",
        {
            ...
            width: 170, emptyText: "--请选择--", autoLoadOnValue: true,
            readOnly : true,
            listeners:
            {
                select: function (combo, records, eOpts) {
                    ...
                    //Ext.Msg.alert("您展开了目录树节点！！！", "您展开了目录树节点！节点ID:" + records[0].data.value);
                    me.send_get_designphase_profession(records[0].data.value, function () {
                    });
                }
            }
        });
    ...
    send_get_created_profession_callback: function (response, options, funCallback) {
        ...
        if (state === true) {
            ...
            if (strCreatedProfession != undefined) {
                ...
                //新建设计阶段
                me.designPhaseCombo.readOnly = false;
    ...
    refreshWin: function (projKeyword, closeWin) {
        ...
        //调用流程页事件，刷新父控件内容
        if (me.startAtWfBtn === "false") {
            //如果是在菜单里面调用本窗口
            Ext.getCmp(me.mainPanelId).refreshWin(projKeyword, function () {
                if (closeWin)
                    winSelectProfession.close();
            });
        } else {
