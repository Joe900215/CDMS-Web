/*批量创建文档*/
Ext.define('Ext.plug_ins.HXEPC_Plugins.Project.BatchCreateContract', {
    extend: 'Ext.container.Container',
    alias: 'widget.BatchCreateContract',
    //layout: "border",
    layout: 'fit',
    ...
    initComponent: function () {
                            ...
                            if (nodes !== null && nodes.length > 0) {
                                ...
                                for (var i = 0; i < arr.length ; i++) {
                                    ...
                                    if (arrItem[0] != "") {
                                        ...
                                        rec.set('contractDesc', arrItem[0]);
                                        rec.set('contractCode', arrItem[1]);
                                        //rec.set('contractDesc', arrItem[2]);
        ...
        Ext.define("contentModel", {
            ...
            fields: ["id", "contractCode", "contractDesc", "tempDef"],
            url: "_blank",
        });
        ...
        me.contentGrid = Ext.create('Ext.grid.Panel', {
            ...
            columns: [
                //{
                //    header: '英文名', dataIndex: 'contractEngDesc',  width: 321,
                //    editor: me.CodeEditing
                //},
                {
                    header: '合同名称', dataIndex: 'contractDesc',  width: 321,
                    editor: me.CodeEditing
                },
                {
                    header: '合同号', dataIndex: 'contractCode', width: 120,
    ...
    send_create_Doc: function () {
        ...
        if (batSend === true) {
            ...
            me.createDocIndex = 0;
            me.loop_create_contract(0);//循环创建文档
    ...
    //循环创建文档
    loop_create_contract: function (docIndex) {
        var me = this;
        var records = me.contentStore.getRange();
        if (docIndex >= records.length) {
            winBatchCreateContract.close();
        } else {
            if (records[docIndex].data.contractCode === "") {
                docIndex = docIndex + 1;
                //循环创建文档
                me.loop_create_contract(docIndex);
            } else {
                contractAttr =
                [{ name: 'contractCode', value: records[docIndex].data.contractCode },
                    { name: 'contractDesc', value: records[docIndex].data.contractDesc },
                    //{ name: 'contractEngDesc', value: records[docIndex].data.contractEngDesc },
                ...
                Ext.Ajax.request({
                    ...
                    method: "POST",
                    params: {
                        C: "AVEVA.CDMS.HXEPC_Plugins.HXProject", A: "CreateContract",
                        sid: localStorage.getItem("sid"), projectKeyword: me.projectKeyword, contractAttrJson: contractAttrJson,
                    },
    ...
    batch_send_create_Doc: function () {
        ...
        for (var i = 0; i < records.length; i++) {
            if (records[i].data.contractCode != "") {
        ...
        Ext.Ajax.request({
            ...
            method: "POST",
            params: {
                C: "AVEVA.CDMS.HXEPC_Plugins.HXProject", A: "BatchCreateContract",
