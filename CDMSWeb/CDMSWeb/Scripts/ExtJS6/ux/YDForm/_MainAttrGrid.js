...
Ext.define('Ext.ux.YDForm._MainAttrGrid', {
    ...
    initComponent: function () {
        ...
        me.DocAttrDataColumn = Ext.create("Ext.grid.column.Column", {
            text: '属性值', dataIndex: 'AttrValue', flex: 1,
        ...
        if (objType === "doc") {
            ...
            Ext.Ajax.request({
                ...
                params: {
                    ...
                    sid: localStorage.getItem("sid"), projectKeyword: Keyword,//action: "ModiProject",
                    ...
                    if (records != undefined && records.length > 0 && records[0] !== null && records[0] !== undefined) {
                        ...
                        me.hasProjectEditRight = "False";
                            for (var i = 0; i < records.length ; i++) {
                                ...
                                } else if (attrCode === "isAdmin") {
                                    me.setIsAdmin(record.data.AttrValue);
                                }
                            }
                        }
                }
    ...
    setIsAdmin: function (value) {
        ...
        //是否显示权限列表
        var view = me.up('_mainAttrTab');
        var viewTree = me.up('_mainSourceView').down('_mainProjectTree');

        if (value === "True") {
            view.setRightTabVisible(true);
            viewTree._editUserButton.show();
            viewTree._editGroupButton.show();
        }
        else {
            view.setRightTabVisible(false);
            viewTree._editUserButton.hide();
            viewTree._editGroupButton.hide();
        }
        me.isAdmin = value;
    }
