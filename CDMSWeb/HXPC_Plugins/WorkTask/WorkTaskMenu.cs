...
{
    internal class WorkTaskMenu : ExWebMenu
    {
        ...
        public override enWebMenuState MeasureMenuState()
        {
            try
            {
                ...
                if (((project != null) && project.Code == "工作任务" && project.ParentProject != null && (project.ParentProject.TempDefn != null)) && (project.ParentProject.TempDefn.Code == "PROFESSION"))
                {

                    //判断是否为主设或者专业设计人
                    bool flag = false;

                    string valueByKeyWord = project.GetValueByKeyWord("PROFESSIONMANAGER");      //主任
                    ...
                    valueByKeyWord = project.GetValueByKeyWord("PROFESSIONOWNER");  //主设
                    if (!string.IsNullOrEmpty(valueByKeyWord) && (valueByKeyWord.ToLower() == project.dBSource.LoginUser.Code.ToLower()))
                    {
                        flag = true;
                    }
